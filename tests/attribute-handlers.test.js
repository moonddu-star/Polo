...
const { html, js, entry } = require('./component-source.js');
...
test('the top button scrolls up from a listener it binds itself', () => {
  ...
  // syncUi re-runs the stateful refs after a state change; binding twice would
  // scroll twice.
  ref(el);
...
test('the trailer starts from a listener it binds itself', () => {
  ...
  const ref = entry('trailerBox')(ctx);
